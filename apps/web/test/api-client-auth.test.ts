...
test('401 from an authenticated API request emits the session-expired event',async()=>{
  ...
  globalThis.fetch=async()=>new Response(JSON.stringify({message:'expired'}),{status:401,headers:{'content-type':'application/json'}})
  try{
    await assert.rejects(()=>api.get('/incidents'),(error:unknown)=>error instanceof ApiError&&error.status===401)
...
test('failed login does not emit a session-expired event',async()=>{
  ...
  browserWindow.addEventListener('assetflow:session-expired',()=>expired++)
  Object.defineProperty(globalThis,'window',{value:browserWindow,configurable:true,writable:true})
  globalThis.fetch=async()=>new Response(JSON.stringify({message:'invalid credentials'}),{status:401,headers:{'content-type':'application/json'}})
  try{
