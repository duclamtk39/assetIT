...
test('category deletion is blocked after business data references it',async()=>{
  const db={assetCategory:{findUnique:async()=>({id:'category-id',code:'LAPTOP',name:'Laptop',status:'ACTIVE',_count:{assets:1,children:0,models:0,inventorySessions:0}})}}
...
test('unused category can be deleted and the action is audited',async()=>{
  ...
  const tx={
    assetCategory:{delete:async()=>{actions.push('delete')}},
    auditLog:{create:async({data}:any)=>{actions.push(data.action)}},
  }
  const db={
    assetCategory:{findUnique:async()=>({id:'category-id',code:'CAMERA',name:'Camera',status:'ACTIVE',_count:{assets:0,children:0,models:0,inventorySessions:0}})},
...
test('category with active children cannot be deactivated',async()=>{
  const db={assetCategory:{findUnique:async()=>({id:'category-id',code:'IT',name:'Thiết bị IT',status:'ACTIVE'}),count:async()=>1,findFirst:async()=>null}}
  const service=new CategoriesService(db as never)
