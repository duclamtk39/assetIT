...
test('Excel staging checks duplicate identifiers only against active assets',async()=>{
  ...
  const tx={assetImportBatch:{create:async({data}:any)=>({id:'batch-1',...data})},assetImportRow:{createMany:async()=>({count:1})},auditLog:{create:async()=>({})}}
  const db={
    asset:{findMany:async({where}:any)=>{duplicateWhere=where;return[]}},
    assetCategory:{findMany:async()=>[{id:valid.categoryId}]},warehouse:{findMany:async()=>[{id:valid.warehouseId}]},
    $transaction:(work:any)=>work(tx),
  }
  const service=new AssetImportsService(db as any)
  const result=await service.stage({sourceFileName:'assets.xlsx',rows:[{rowNumber:2,payload:valid}]} as any,{id:'admin',role:'ADMIN',departmentId:null})
