export type DisposalWorkflowStatus =
  'DRAFT' | 'SUBMITTED' | 'APPROVED' | 'REJECTED' | 'IN_EXECUTION' | 'COMPLETED' | 'CANCELLED'
...
export function assertCanComplete(
  ...
  items: Array<{ requiresDataSanitization: boolean; sanitizationStatus: string }>,
) {
  ...
  if (items.some(item => item.requiresDataSanitization && item.sanitizationStatus !== 'VERIFIED'))
    throw new Error('DATA_SANITIZATION_REQUIRED')
}
