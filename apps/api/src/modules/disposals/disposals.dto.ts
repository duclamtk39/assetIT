...
import { ArrayMinSize,IsArray,IsBoolean,IsDateString,IsEnum,IsInt,IsNotEmpty,IsNumber,IsOptional,IsString,IsUUID,Length,Matches,Max,MaxLength,Min,ValidateNested } from 'class-validator'
...
export class CreateDisposalDto{
  ...
  @IsArray() @ArrayMinSize(1) @ValidateNested({each:true}) @Type(()=>CreateDisposalItemDto) items!:CreateDisposalItemDto[]
}
