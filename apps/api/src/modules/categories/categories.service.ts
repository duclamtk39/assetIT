...
export class CategoriesService{
  ...
  list(){return this.db.assetCategory.findMany({include:{parent:{select:{id:true,code:true,name:true}},_count:{select:{assets:true,children:true,models:true,inventorySessions:true}}},orderBy:[{status:'asc'},{name:'asc'}]})}
  ...
  async update(id:string,body:UpdateAssetCategoryDto,actor:Actor){
    ...
    if(body.code||body.name){
      const duplicate=await this.db.assetCategory.findFirst({where:{id:{not:id},OR:[...(body.code?[{code:{equals:body.code,mode:'insensitive' as const}}]:[]),...(body.name?[{name:{equals:body.name,mode:'insensitive' as const}}]:[])]}})
      if(duplicate)throw new ConflictException('Mã hoặc tên nhóm tài sản đã tồn tại')
    }
    ...
    try{
      return await this.db.$transaction(async tx=>{
        ...
        await tx.auditLog.create({data:{userId:actor.id,action:'ASSET_CATEGORY_UPDATED',entityType:'AssetCategory',entityId:id,oldValues:{code:current.code,name:current.name,parentId:current.parentId,description:current.description,status:current.status} as Prisma.InputJsonValue,newValues:{code:category.code,name:category.name,parentId:category.parentId,description:category.description,status:category.status} as Prisma.InputJsonValue}})
        return category
      })
    }catch(error:any){if(error?.code==='P2002')throw new ConflictException('Mã hoặc tên nhóm tài sản đã tồn tại');throw error}
  }

  async remove(id:string,actor:Actor){
    const category=await this.db.assetCategory.findUnique({where:{id},include:{_count:{select:{assets:true,children:true,models:true,inventorySessions:true}}}})
    if(!category)throw new NotFoundException('Không tìm thấy nhóm tài sản')
    const references=category._count.assets+category._count.children+category._count.models+category._count.inventorySessions
    if(references)throw new ConflictException('Nhóm đã phát sinh dữ liệu nên không thể xóa. Hãy chọn Ngừng sử dụng để bảo toàn lịch sử.')
    await this.db.$transaction(async tx=>{
      await tx.assetCategory.delete({where:{id}})
      await tx.auditLog.create({data:{userId:actor.id,action:'ASSET_CATEGORY_DELETED',entityType:'AssetCategory',entityId:id,oldValues:{code:category.code,name:category.name,status:category.status} as Prisma.InputJsonValue}})
    })
    return {success:true}
  }
}
