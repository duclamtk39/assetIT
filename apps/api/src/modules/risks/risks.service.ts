...
export class RisksService {
  ...
  private assertOperator(actor: Actor) {
    if (!['ADMIN', 'IT'].includes(actor.role))
      throw new ForbiddenException('Chỉ Admin hoặc IT được quản lý đánh giá rủi ro CNTT')
  }
  ...
  private async assertUser(id: string, label: string) {
    const user = await this.db.user.findFirst({
      where: { id, status: 'ACTIVE', role: { in: ['ADMIN', 'IT'] } },
      select: { id: true },
    })
  ...
  async operators(actor: Actor) {
    ...
    return this.db.user.findMany({
      where: { status: 'ACTIVE', role: { in: ['ADMIN', 'IT'] } },
      select: {
        id: true,
        employeeCode: true,
        fullName: true,
  ...
  private async validateAssessmentReferences(body: CreateRiskAssessmentDto) {
    ...
    if (body.approverId) {
      ...
      if (body.approverId === body.ownerId)
        throw new BadRequestException('Người phê duyệt phải độc lập với chủ sở hữu đợt đánh giá')
    }
    if (
      body.departmentId &&
      !(await this.db.department.findFirst({
        where: { id: body.departmentId, status: 'ACTIVE' },
        select: { id: true },
      }))
    )
      throw new BadRequestException('Phòng ban trong phạm vi đánh giá không hợp lệ')
    const start = new Date(body.startDate),
      target = this.date(body.targetDate),
      review = this.date(body.nextReviewAt)
  ...
  async list(query: ListRiskAssessmentsQuery, actor: Actor) {
    ...
    const where: Prisma.RiskAssessmentWhereInput = {
      ...
      OR: text ? [{ assessmentNo: text }, { title: text }, { scope: text }] : undefined,
    }
    const [data, total] = await this.db.$transaction([
      this.db.riskAssessment.findMany({
        where,
        include: {
  ...
  async getAssessment(id: string, actor: Actor) {
    this.assertOperator(actor)
    const assessment = await this.db.riskAssessment.findUnique({
      where: { id },
      include: {
        owner: { select: { id: true, fullName: true, email: true } },
        approver: { select: { id: true, fullName: true, email: true } },
        department: { select: { id: true, code: true, name: true } },
        risks: { include: riskInclude, orderBy: [{ inherentScore: 'desc' }, { createdAt: 'desc' }] },
  ...
  async createAssessment(body: CreateRiskAssessmentDto, actor: Actor) {
    ...
    return this.db.$transaction(async tx => {
      const assessment = await tx.riskAssessment.create({
        data: {
          assessmentNo: this.reference('DGRR'),
          ...
          targetDate: this.date(body.targetDate),
        ...
        text
          ? {
              OR: [
                ...
                { threat: text },
                { vulnerability: text },
                { assets: { some: { asset: { OR: [{ assetTag: text }, { name: text }, { serialNumber: text }] } } } },
              ],
            }
          : {},
        query.level
  ...
  private async validateRiskReferences(body: CreateRiskItemDto | UpdateRiskItemDto) {
    ...
    if (
      ...
      throw new BadRequestException('Phòng ban chịu ảnh hưởng không hợp lệ')
    if ('assetIds' in body && body.assetIds.length) {
      ...
      if (count !== new Set(body.assetIds).size)
        throw new BadRequestException('Danh sách tài sản liên quan có dữ liệu không hợp lệ')
    }
    if ('incidentIds' in body && body.incidentIds.length) {
      const count = await this.db.incident.count({ where: { id: { in: body.incidentIds } } })
      if (count !== new Set(body.incidentIds).size)
        throw new BadRequestException('Danh sách sự cố liên quan có dữ liệu không hợp lệ')
    }
  }
  ...
  async createRisk(assessmentId: string, body: CreateRiskItemDto, actor: Actor) {
    ...
    if (!assessment) throw new NotFoundException('Không tìm thấy đợt đánh giá rủi ro')
    if (['CLOSED', 'CANCELLED'].includes(assessment.status))
      throw new BadRequestException('Đợt đánh giá đã đóng hoặc hủy, không thể thêm rủi ro')
    const inherent = calculateRiskScore(body.likelihood, body.impact)
    const residual =
      body.residualLikelihood && body.residualImpact
        ? calculateRiskScore(body.residualLikelihood, body.residualImpact)
        : null
    if (body.treatmentStrategy === 'ACCEPT' && (!body.acceptanceRationale || !residual))
      throw new BadRequestException('Chấp nhận rủi ro phải có lý do và điểm rủi ro còn lại')
    return this.db.$transaction(async tx => {
      const risk = await tx.riskItem.create({
        data: {
          riskNo: this.reference('RR'),
          assessmentId,
          title: body.title.trim(),
          category: body.category.trim(),
          scenario: body.scenario.trim(),
          threat: body.threat.trim(),
          vulnerability: body.vulnerability.trim(),
          existingControls: this.clean(body.existingControls),
          source: body.source,
          ...
          acceptanceRationale: this.clean(body.acceptanceRationale),
          ownerId: body.ownerId,
          departmentId: body.departmentId,
      ...
      if (body.assetIds.length)
        await tx.riskAsset.createMany({
          data: [...new Set(body.assetIds)].map(assetId => ({ riskId: risk.id, assetId })),
        })
      if (body.incidentIds.length)
        await tx.riskIncident.createMany({
          data: [...new Set(body.incidentIds)].map(incidentId => ({ riskId: risk.id, incidentId })),
        })
      await tx.auditLog.create({
        data: {
          userId: actor.id,
  ...
  async updateRisk(id: string, body: UpdateRiskItemDto, actor: Actor) {
    ...
    if (current.status === 'CLOSED')
      throw new BadRequestException('Rủi ro đã đóng chỉ được phép đọc và rà soát lịch sử')
    ...
    const residualLikelihood = body.residualLikelihood ?? current.residualLikelihood,
      residualImpact = body.residualImpact ?? current.residualImpact
    const residual =
      residualLikelihood && residualImpact ? calculateRiskScore(residualLikelihood, residualImpact) : null
    const strategy = body.treatmentStrategy ?? current.treatmentStrategy,
      rationale = body.acceptanceRationale ?? current.acceptanceRationale
    if (body.status === 'ACCEPTED') {
      if (actor.role !== 'ADMIN') throw new ForbiddenException('Chỉ Admin được phê duyệt chấp nhận rủi ro còn lại')
      if (strategy !== 'ACCEPT' || !rationale || !residual)
        throw new BadRequestException('Chấp nhận rủi ro phải có chiến lược ACCEPT, lý do và điểm rủi ro còn lại')
    }
    const data: Prisma.RiskItemUpdateInput = {
      ...
      threat: body.threat?.trim(),
      vulnerability: body.vulnerability?.trim(),
      existingControls: body.existingControls === undefined ? undefined : this.clean(body.existingControls),
      status: body.status,
      likelihood,
      impact,
      inherentScore: inherent.score,
      ...
      acceptanceRationale: body.acceptanceRationale === undefined ? undefined : this.clean(body.acceptanceRationale),
      owner: body.ownerId ? { connect: { id: body.ownerId } } : undefined,
      department: body.departmentId ? { connect: { id: body.departmentId } } : undefined,
      dueDate: body.dueDate === undefined ? undefined : this.date(body.dueDate),
      nextReviewAt: body.nextReviewAt === undefined ? undefined : this.date(body.nextReviewAt),
    }
    return this.db.$transaction(async tx => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          oldValues: {
            ...
            inherentScore: current.inherentScore,
            residualScore: current.residualScore,
  ...
  async addControl(riskId: string, body: CreateRiskControlDto, actor: Actor) {
    this.assertOperator(actor)
    if (!(await this.db.riskItem.findUnique({ where: { id: riskId }, select: { id: true } })))
      throw new NotFoundException('Không tìm thấy hồ sơ rủi ro')
    return this.db.$transaction(async tx => {
      const control = await tx.riskControl.create({
        data: {
          riskId,
          controlCode: this.clean(body.controlCode),
          title: body.title.trim(),
          description: this.clean(body.description),
          framework: this.clean(body.framework),
          status: body.status,
          effectiveness: body.effectiveness,
          evidence: this.clean(body.evidence),
        },
      })
      await tx.auditLog.create({
        data: {
          userId: actor.id,
          action: 'RISK_CONTROL_ADDED',
          entityType: 'RiskItem',
          entityId: riskId,
          newValues: {
  ...
  async addTreatment(riskId: string, body: CreateRiskTreatmentDto, actor: Actor) {
    ...
    if (!risk) throw new NotFoundException('Không tìm thấy hồ sơ rủi ro')
    if (['ACCEPTED', 'CLOSED'].includes(risk.status))
      throw new BadRequestException('Rủi ro đã chấp nhận hoặc đóng, không thể thêm hành động xử lý')
    return this.db.$transaction(async tx => {
      const treatment = await tx.riskTreatmentAction.create({
        data: {
          riskId,
          title: body.title.trim(),
          description: this.clean(body.description),
      ...
      await tx.auditLog.create({
        data: {
          ...
          newValues: {
            ...
            assigneeId: treatment.assigneeId,
  ...
  async updateTreatment(id: string, body: UpdateRiskTreatmentDto, actor: Actor) {
    ...
    if (actor.role !== 'ADMIN' && current.assigneeId !== actor.id)
      throw new ForbiddenException('Chỉ người được giao hoặc Admin được cập nhật hành động xử lý')
    const status = body.status ?? current.status,
      progress = status === 'COMPLETED' ? 100 : (body.progress ?? current.progress)
    if (status === 'COMPLETED' && !body.outcome && !current.outcome)
      throw new BadRequestException('Hoàn tất hành động xử lý phải ghi nhận kết quả')
    return this.db.$transaction(async tx => {
      const updated = await tx.riskTreatmentAction.update({
        where: { id },
        data: {
          status,
          progress,
          outcome: body.outcome === undefined ? undefined : this.clean(body.outcome),
          completedAt: status === RiskTreatmentStatus.COMPLETED ? new Date() : null,
        },
      })
      const open = await tx.riskTreatmentAction.count({
        where: { riskId: current.riskId, status: { in: ['PLANNED', 'IN_PROGRESS'] } },
      })
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityType: 'RiskItem',
          entityId: current.riskId,
          oldValues: { treatmentId: id, status: current.status, progress: current.progress } as Prisma.InputJsonValue,
          newValues: { status: updated.status, progress: updated.progress } as Prisma.InputJsonValue,
        },
      })
      return updated
    })
  ...
  async reviewAssessment(id: string, body: ReviewRiskDto, actor: Actor) {
    ...
    if (body.decision === RiskReviewDecision.APPROVE && current.status === 'IN_REVIEW') {
      if (current.approverId && current.approverId !== actor.id)
        throw new ForbiddenException('Đợt đánh giá phải được người phê duyệt đã chỉ định xử lý')
    ...
    try {
      status = assessmentStatusAfterDecision(current.status, body.decision)
    } catch {
      throw new BadRequestException('Quyết định phê duyệt không phù hợp với trạng thái hiện tại')
    }
    const now = new Date()
    return this.db.$transaction(async tx => {
      const updated = await tx.riskAssessment.update({
        ...
        data: {
          status,
          submittedAt: body.decision === 'SUBMIT' ? now : undefined,
          approvedAt: body.decision === 'APPROVE' ? now : undefined,
          closedAt: body.decision === 'CLOSE' ? now : undefined,
        },
      })
      await tx.riskReview.create({
        data: { assessmentId: id, decision: body.decision, note: body.note.trim(), reviewedBy: actor.id },
      })
      await tx.auditLog.create({
        data: {
          userId: actor.id,
          action: 'RISK_ASSESSMENT_REVIEWED',
          entityType: 'RiskAssessment',
          entityId: id,
          oldValues: { status: current.status } as Prisma.InputJsonValue,
          newValues: { status, decision: body.decision } as Prisma.InputJsonValue,
        },
      })
      return updated
    })
  ...
  async reviewRisk(id: string, body: ReviewRiskDto, actor: Actor) {
    ...
    if (!allowedDecisions.includes(body.decision))
      throw new BadRequestException('Quyết định rà soát rủi ro không hợp lệ')
    ...
    return this.db.$transaction(async tx => {
      ...
      await tx.riskReview.create({
        data: { riskId: id, decision: body.decision, note: body.note.trim(), reviewedBy: actor.id },
      })
      await tx.auditLog.create({
        data: {
          userId: actor.id,
